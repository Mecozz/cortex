...
#[tauri::command]
pub fn get_brain_status(state: State<DbState>, watch: State<WatchState>) -> watch::BrainStatus {
    use crate::core::health::HealthCheck;

    let cb = watch.0.lock().ok();
    let mut modules = vec![
        crate::memory::health::MemoryHealth.health(),
        crate::tasks::health::TaskHealth.health(),
        crate::inject::health::InjectHealth.health(),
        crate::cost::health::CostHealth.health(),
        crate::librarian::health::LibrarianHealth.health(),
        crate::watch::health::WatchHealth.health(),
    ];

    // Check providers with CB state
    let has_key = state
        .0
        .lock()
        .map(|conn| {
    ...
    modules.push(
        crate::providers::health::ProvidersHealth {
    ...
    let module_statuses: Vec<watch::ModuleHealth> = modules
        .into_iter()
        .map(|r| {
    ...
    let overall = if module_statuses
        .iter()
        .any(|m| m.disabled || m.status == "red")
    {
        "red".into()
    } else if module_statuses.iter().any(|m| m.status == "yellow") {
        "yellow".into()
    } else {
        "green".into()
    };
